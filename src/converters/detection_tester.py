...
for i in range(iters):
    tf0 = time.time()
    boxes, landmarks = detector.detect(image.transformed_image, threshold=0.6)
    for i in range(len(boxes)):
        bbox = reproject_points(boxes[i], image.scale_factor)
        landmark = reproject_points(landmarks[i], image.scale_factor)
        eyes = landmark[0:2]
        _crop = fa.align(image.orig_image, eyes)
        # _crop = face_align.norm_crop(image.orig_image, landmark=landmark)
        cv2.imwrite('align_result.jpg', _crop)
    tf1 = time.time()
